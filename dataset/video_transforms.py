...
class UniformTemporalSubsample(torch.nn.Module):
	...
	def __init__(self, num_samples: int, temporal_dim: int = 1,skip_key = ['label']):
		"""
		Args:
	...
	def forward(self,datadict ) :
		...
		"""
		return self.uniform_temporal_subsample(datadict,
											   self._num_samples,
											   self._temporal_dim)
	
	def uniform_temporal_subsample(
		self,
		datadict, num_samples: int, temporal_dim: int = 1
	) -> torch.Tensor:
		"""
		...
		for key,value in datadict.items():    
			...
			datadict[key] =  torch.index_select(value, cur_temporal_dim, indices)
